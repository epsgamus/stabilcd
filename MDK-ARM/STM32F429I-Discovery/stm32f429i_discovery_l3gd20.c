...
/**
  ...
  *         This value could be:
  *           - L3GD20_X_INTERRUPT_ENABLE: Interrupt on X is activated
  *           - L3GD20_Y_INTERRUPT_ENABLE: Interrupt on Y is activated
  *           - L3GD20_Z_INTERRUPT_ENABLE: Interrupt on Z is activated
  *           - L3GD20_AXES_INTERRUPT_ENABLE: Interrupt on X, Y and Z are activated
  * @retval None
  */
void L3GD20_INT1InterruptConfig(uint8_t Interrupt_Axes)
{
  ...
  if(Interrupt_Axes == L3GD20_X_INTERRUPT_ENABLE)
  {
    L3GD20_Write(&regval[0], L3GD20_INT1_TSH_XH_ADDR, 1); 
  }
  else if (Interrupt_Axes == L3GD20_Y_INTERRUPT_ENABLE)
  {
    L3GD20_Write(&regval[0], L3GD20_INT1_TSH_YH_ADDR, 1); 
  }
  else if (Interrupt_Axes == L3GD20_Z_INTERRUPT_ENABLE) 
  {
    L3GD20_Write(&regval[0], L3GD20_INT1_TSH_ZH_ADDR, 1); 
  }
  else
  {
    L3GD20_Write(&regval[0], L3GD20_INT1_TSH_XH_ADDR, 1); 
  ...
  L3GD20_Write(&regval[1], L3GD20_INT1_TSH_XL_ADDR, 1); 
  L3GD20_Write(&regval[1], L3GD20_INT1_TSH_YL_ADDR, 1); 
...
{ 
  ...
  SYSCFG_EXTILineConfig(L3GD20_SPI_INT1_EXTI_PORT_SOURCE, L3GD20_SPI_INT1_EXTI_PIN_SOURCE);  
  
  /* Configure INT1 EXTI line */
  EXTI_InitStructure.EXTI_Line = L3GD20_SPI_INT1_EXTI_LINE;
  ...
  NVIC_InitStructure.NVIC_IRQChannel = L3GD20_SPI_INT1_EXTI_IRQn;
