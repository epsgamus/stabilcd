...
void LCD_LayerInit(void)
{
  ...
  */
  LTDC_Layer_InitStruct.LTDC_CFBLineLength = ((LCD_SIZE_PIXEL_WIDTH * 2) + 3);
  /* the pitch is the increment from the start of one line of pixels to the 
  start of the next line in bytes, then :
  Pitch = Active high width x number of bytes per pixel */ 
  LTDC_Layer_InitStruct.LTDC_CFBPitch = (LCD_SIZE_PIXEL_WIDTH * 2);
...
void LCD_ClearLine(uint16_t Line)
{
  ...
  while ((refcolumn < LCD_SIZE_PIXEL_WIDTH) && (((refcolumn + LCD_Currentfonts->Width)& 0xFFFF) >= LCD_Currentfonts->Width))
  {
...
{  
  return CurrentFrameBuffer + 2*(Xpos + (LCD_SIZE_PIXEL_WIDTH*Ypos));
}
...
void LCD_DrawChar(uint16_t Xpos, uint16_t Ypos, const uint16_t *c)
{
  ...
  xpos = Xpos*LCD_SIZE_PIXEL_WIDTH*2;
  Xaddress += Ypos;
  ...
  for(index = 0; index < LCD_Currentfonts->Height; index++)
  {
    ...
    for(counter = 0; counter < LCD_Currentfonts->Width; counter++)
    {
      ...
      Xaddress += (LCD_SIZE_PIXEL_WIDTH - LCD_Currentfonts->Width);
  }
}
...
{  
  uint16_t refcolumn = 0;
  /* Send the string character by character on lCD */
  while ((refcolumn < LCD_SIZE_PIXEL_WIDTH) && ((*ptr != 0) & (((refcolumn + LCD_Currentfonts->Width) & 0xFFFF) >= LCD_Currentfonts->Width)))
  {
    /* Display one character on LCD */
...
{
  LCD_SetDisplayWindow(0, 0, LCD_SIZE_PIXEL_HEIGHT, LCD_SIZE_PIXEL_WIDTH); 
}
...
void LCD_DrawLine(uint16_t Xpos, uint16_t Ypos, uint16_t Length, uint8_t Direction)
{
  ...
  Xaddress = CurrentFrameBuffer + 2*(LCD_SIZE_PIXEL_WIDTH*Ypos + Xpos);
  ...
  {                                                            
    DMA2D_InitStruct.DMA2D_OutputOffset = LCD_SIZE_PIXEL_WIDTH - 1;                
...
void LCD_DrawCircle(uint16_t Xpos, uint16_t Ypos, uint16_t Radius)
{
    ...
    do {
        *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos-x) + LCD_SIZE_PIXEL_WIDTH*(Ypos+y)))) = CurrentTextColor; 
        *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos+x) + LCD_SIZE_PIXEL_WIDTH*(Ypos+y)))) = CurrentTextColor;
        *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos+x) + LCD_SIZE_PIXEL_WIDTH*(Ypos-y)))) = CurrentTextColor;
        *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos-x) + LCD_SIZE_PIXEL_WIDTH*(Ypos-y)))) = CurrentTextColor;
...
void LCD_DrawEllipse(int Xpos, int Ypos, int Radius, int Radius2)
{
  ...
  { 
    do {
      K = (float)(rad1/rad2);
      *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos-x) + LCD_SIZE_PIXEL_WIDTH*(Ypos+(uint16_t)(y/K))))) = CurrentTextColor; 
      *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos+x) + LCD_SIZE_PIXEL_WIDTH*(Ypos+(uint16_t)(y/K))))) = CurrentTextColor;
      *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos+x) + LCD_SIZE_PIXEL_WIDTH*(Ypos-(uint16_t)(y/K))))) = CurrentTextColor;
      *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos-x) + LCD_SIZE_PIXEL_WIDTH*(Ypos-(uint16_t)(y/K))))) = CurrentTextColor;
  ...
  else
  {
    ...
    do { 
      K = (float)(rad2/rad1);
      *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos-(uint16_t)(x/K)) + LCD_SIZE_PIXEL_WIDTH*(Ypos+y)))) = CurrentTextColor; 
      *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos+(uint16_t)(x/K)) + LCD_SIZE_PIXEL_WIDTH*(Ypos+y)))) = CurrentTextColor;
      *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos+(uint16_t)(x/K)) + LCD_SIZE_PIXEL_WIDTH*(Ypos-y)))) = CurrentTextColor;
      *(__IO uint16_t*) (CurrentFrameBuffer + (2*((Xpos-(uint16_t)(x/K)) + LCD_SIZE_PIXEL_WIDTH*(Ypos-y)))) = CurrentTextColor;
...
void LCD_DrawMonoPict(const uint32_t *Pict)
{
  ...
  for(index = 0; index < 2400; index++)
  {
    for(counter = 0; counter < 32; counter++)
    {
      ...
      {
        *(__IO uint16_t*)(CurrentFrameBuffer + index*32 + counter) = CurrentBackColor;
      }
      else
      {
        *(__IO uint16_t*)(CurrentFrameBuffer + index*32 + counter) = CurrentTextColor;
      }
    }
  }
}
...
void LCD_DrawFullRect(uint16_t Xpos, uint16_t Ypos, uint16_t Width, uint16_t Height)
{
  ...
  DMA2D_InitStruct.DMA2D_OutputAlpha = 0x0F;                  
  DMA2D_InitStruct.DMA2D_OutputMemoryAdd = Xaddress;                
  DMA2D_InitStruct.DMA2D_OutputOffset = (LCD_SIZE_PIXEL_WIDTH - Width);                
